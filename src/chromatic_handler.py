...
class ChromaticHandler:
    ...
    def retry_chromatic(self, pr):
        try:
            ...
            if retried_any:
                self.logger.info(f"🔍 Looking for Chromatic workflows to retry...")
                ...
                if "chromatic" in workflow.name.lower():
                    ...
                    for run in runs:
                        if run.head_sha == pr.head.sha and run.conclusion == "failure":
                            self.logger.info(f"🔄 Re-running workflow: {workflow.name} (run #{run.id})")
                            try:
                                run.rerun()
            ...
            if not retried_any:
                self.logger.info(f"ℹ️  No failed Chromatic checks found to retry for PR #{pr.number}")
